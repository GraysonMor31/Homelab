...
### 2. Networking
- **TwinGate**: Open-source ZTNA system deployed across all devices for home network access and provides network logs.
- **Caddy Reverse Proxy**: Dynamic reverse proxy and load balancer for managing containerized services.
...
- **ShuffleSOAR**: Open-source Security Orchestration, Automation, and Response system.
- **Suricata**:
- **TheHIVE**:
- **MISP**:
- **Eramba CE**:
...
- **Terraform**: Infrastructure as code platform to automate server deployment and scaling
...
- **Drone**: Self-hosted CI/CD pipeline.
...
- **VaultWarden**: A self-hosted password manager that works with most browsers.

## Architecture Overview
This homelab is built with the following components:
1. **Host Hardware**: Servers (physical or virtualized) running Proxmox, this includes a cluster of old Intel-based HP workstations, a few Raspberry Pis clustered together, my desktop, and my laptop
...
5. **Databases**: Every service that requires a database will be using one built into the Docker stack that is running the service. Most use SQLite; certain high-criticality services use PostgreSQL or MariaDB.
